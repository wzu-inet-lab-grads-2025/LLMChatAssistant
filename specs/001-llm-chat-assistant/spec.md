...
- Q: AGENT_THOUGHT 消息发送时机 → A: 根据实际任务复杂度合理设计（简单任务可批量发送，复杂任务可实时发送以保持用户体验）
...
3. **给定** 传输过程中发生丢包，**当** 发送方超时未收到ACK，**那么** 系统应自动重传丢失的数据包
...
- **FR-004**: 服务器端Agent必须实现ReAct循环，自动化处理所有前端消息（无需前端触发），通过工具链得到结果后返回最终回复，支持最多5轮工具调用
...
- **FR-009a**: 系统必须支持多会话管理功能，允许用户创建、查看、切换和删除多个命名会话
- **FR-009b**: 系统必须为每个会话自动生成基于对话主题的名称（使用 AI 分析前几轮对话生成标题）
- **FR-009c**: 系统必须支持 /sessions、/switch `<session_id>`、/new、/delete `<session_id>` 命令用于多会话管理
- **FR-009d**: 会话切换时必须加载完整的压缩上下文和上下文窗口标记（符合双层管理机制）
- **FR-009e**: 系统必须支持会话自动归档功能（超过 30 天未访问的会话自动归档到 storage/history/archive/）
...
- **FR-014c**: 系统必须在启动时验证 config.yaml 存在且格式正确，缺失 .env 时给出明确错误提示
- **FR-014d**: 系统必须支持配置优先级：命令行参数 > 环境变量 > config.yaml，允许环境变量覆盖配置文件中的设置
...
- **Agent会话**: 单用户与AI的对话上下文，包含历史消息和工具调用结果，持久化到 storage/history/ 目录，支持跨会话的上下文延续
...
- **SC-007**: 系统架构支持扩展到多客户端场景（当前实现为单用户场景，未来扩展目标为 10 个并发客户端）
