...
3. **给定** 上传的文件超过1MB，**当** 系统计算Embedding，**那么** 文件应被分块处理并每块独立建立向量索引
...
- 当智谱API调用失败（如配额用尽），系统应降级为本地命令执行模式
...
- **FR-004**: 服务器端Agent必须实现ReAct循环，支持最多5轮工具调用
...
- **FR-020**: CLI 必须提供模型切换命令，允许用户在 glm-4-flash 和 glm-4.5-flash 之间动态切换
